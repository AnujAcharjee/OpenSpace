...
export function EmojiPicker({
  ...
  return (
    <div
      ref={containerRef}
      className={`absolute bottom-full mb-3 left-0 sm:left-2 z-50 flex flex-col w-80 sm:w-[340px] max-h-[380px] rounded-2xl border border-border/60 bg-card/95 shadow-[0_16px_48px_rgba(0,0,0,0.35)] dark:border-[#d4af37]/30 backdrop-blur-2xl p-3 text-foreground animate-in fade-in zoom-in-95 duration-150 select-none ${className}`}
      role="dialog"
      aria-label="Emoji picker"
    >
      {/* Quick Reaction Bar */}
      <div className="flex items-center justify-between gap-1 pb-2 border-b border-border/40 mb-2">
        <span className="text-[10px] font-semibold text-muted-foreground tracking-wider uppercase pl-1">
          Quick
        </span>
        <div className="flex items-center gap-1">
          {QUICK_REACTIONS.map((emoji) => (
            <button
              ...
              className="flex h-7 w-7 items-center justify-center rounded-lg text-base transition-all duration-150 hover:scale-125 hover:bg-primary/15 active:scale-95 cursor-pointer"
            >
              {emoji}
            </button>
          ))}
      ...
      <div className="relative flex items-center mb-2">
        <div className="flex w-full items-center gap-2 rounded-xl border border-border/50 bg-muted/60 px-3 py-1.5 text-xs transition-colors focus-within:border-primary/60 focus-within:bg-muted/80">
      ...
      {!searchQuery && (
        <div className="flex items-center gap-1 border-b border-border/40 pb-2 mb-2 overflow-x-auto no-scrollbar">
          {EMOJI_CATEGORIES.map((cat, idx) => (
            <button
              key={cat.name}
              type="button"
              onClick={() => setActiveCategoryIndex(idx)}
              className={`flex h-7 items-center gap-1.5 rounded-lg px-2.5 text-xs font-medium transition-all duration-150 cursor-pointer ${
      ...
      {/* Emoji Grid Area */}
      <div className="flex-1 overflow-y-auto px-1 max-h-48 pr-1.5 scrollbar-thin scrollbar-thumb-muted-foreground/20">
        {filteredEmojis !== null ? (
          <div>
              ...
              <div className="grid grid-cols-7 sm:grid-cols-8 gap-1">
                {filteredEmojis.map((item, i) => (
                  <button
                    ...
                    title={item.name}
                    className="flex h-9 w-9 items-center justify-center rounded-xl text-xl transition-all duration-150 hover:scale-125 hover:bg-primary/15 hover:shadow-xs active:scale-95 cursor-pointer"
                  >
                    {item.emoji}
                  </button>
                ))}
      ...
      <div className="flex items-center gap-2 pt-2 mt-1 border-t border-border/40 px-1 text-xs text-muted-foreground min-h-7">
        {hoveredEmoji ? (
          <div className="flex items-center gap-2 animate-in fade-in duration-100">
            ...
            <span className="text-[11px] font-medium text-foreground truncate max-w-[240px]">
              {hoveredEmoji.name}
            </span>
          </div>
        ) : (
