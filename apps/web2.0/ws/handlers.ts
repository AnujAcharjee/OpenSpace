...
export const handlers: Record<string, (payload: unknown) => void> = {
  chat_message: (payload) => {
    try {
      ...
      const { addMessage, updateRoomLastMessage, activeRoom, incrementRoomUnread } =
      ...
      console.warn("Failed to parse incoming chat_message:", e)
    }
  },
  ...
  room_deleted: (payload) => {
    try {
      ...
      const { removeRoom, clearMessages, activeRoom, setActiveRoom } =
        useAppStore.getState()
  ...
  room_member_removed: (payload) => {
    try {
      ...
      const { user, removeRoom, clearMessages, activeRoom, setActiveRoom } =
        useAppStore.getState()
